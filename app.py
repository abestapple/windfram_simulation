...
def cw_rotate(tx,ty,x, y, ang):
    ...
    return new_x, new_y
def plot_turbine(ax,turbine_sites,direction,D):
	for i in range(len(turbine_sites)):
		tx=float(turbine_sites[i][0])
		ty=float(turbine_sites[i][1])
		d=float(D[i])
		x1,x2=cw_rotate(tx,ty,tx+d/2, ty, direction)
		x3,x4=cw_rotate(tx,ty,tx-d/2, ty, direction)
		ax.plot([x1,x3],[x2,x4],color="k",lw=2)
		ax.text(tx,ty,i)
	...
	if option not in ["Modified_Park","Park"]:
		...
		if genre=="固定值":
			...
			for xi in display_pos:			
				trace = go.Scatter(
					x = r/D[turbine_id],
					y = wakemodel(np.array(xi),r,velocity,D[turbine_id],ct),
					mode = 'lines + markers',
			...
			for xi in display_pos:
				trace = go.Scatter(
					x = r/D[turbine_id],
					y = wakemodel(np.array(xi),r,ustorge[turbine_id],D[turbine_id],ctstorge[turbine_id]),
					mode = 'lines + markers',
					name = '{}D'.format(xi/D[turbine_id])
				)
				trace_all.append(trace)
