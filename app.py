...
from windfarm_sim.wake_deficit import guass_Bastankhah,guass_Ge,Park,Modified_Park
...
@st.cache_data
def Creat(nx,ny,dx,dy,velocity,direction,turbine_sites,D,ct,wakemodel,superpositionModel):
	...
	wind_farm(flowfield,mesh,turbine_sites,direction,D,ct,wakemodel,superpositionModel)
	fig, ax = plt.subplots(figsize=figsize)
	plot_turbine(ax,turbine_sites,direction,D)
...
@st.cache_data
def Creat_powercurver(nx,ny,dx,dy,velocity,direction,turbine_sites,D,wtg_file,wakemodel,superpositionModel):
	flowfield=Flowfield(velocity,nx,ny,dx,dy)
	#flowfield.change_flow([10,20],[50,30],[11,11])
	mesh=Mesh(nx,ny,dx,dy)
	radio=nx/ny
	figsize=(8, 6.9/radio) if radio>=1 else (4.5*radio,4)
	wind_farm_powercurver(flowfield,mesh,turbine_sites,direction,D,wtg_file,wakemodel,superpositionModel)
	fig, ax = plt.subplots(figsize=figsize)
	plot_turbine(ax,turbine_sites,direction,D)
	cb=ax.contourf(mesh.x,mesh.y,flowfield.flow,levels=50,cmap="Spectral")
	ax.set_xlim([mesh.x.min(),mesh.x.max()])
...
if optionflow=="单风速/风向":
	...
	if genre=="固定值":
		ct=float(ct)
		fig=Creat(nx,ny,dx,dy,velocity,direction,turbine_sites,D,ct,wakemodel,option_superpos)
	else:
		fig=Creat_powercurver(nx,ny,dx,dy,velocity,direction,turbine_sites,D,power_curver,wakemodel,option_superpos)
	...
	if option not in ["Modified_Park","Park"]:
		...
		if genre=="固定值":
			trace_all=[]
			u_st1=VPCT_Turbines_ct(turbine_sites,D,ct,velocity,direction,wakemodel,option_superpos)
			for xi in display_pos:
				model=wakemodel(u_st1[turbine_id],D[turbine_id],ct)			
				trace = go.Scatter(
					x = r/D[turbine_id],
					y = model.deficit_(np.array(xi),r),
					mode = 'lines + markers',
			...
			trace_all=[]
			u_st,p_st,ct_st=VPCT_Turbines(turbine_sites,D,power_curver,velocity,direction,wakemodel,option_superpos)
			for xi in display_pos:
				model=wakemodel(u_st[turbine_id],D[turbine_id],ct_st[turbine_id])
				trace = go.Scatter(
					x = r/D[turbine_id],
					y = model.deficit_(np.array(xi),r),
					mode = 'lines + markers',
	...
	if uploaded_series is not None:
		...
		if genre=="固定值":
			...
			ct=float(ct)
			fig=Creat(nx,ny,dx,dy,series.U[st.session_state["ind"]],series.DIR[st.session_state["ind"]],turbine_sites,D,ct,wakemodel,option_superpos)
			st.pyplot(fig,use_container_width=False)
			...
			if 'ind' not in st.session_state:
				st.session_state["ind"]=list(series.index)[0]
			fig=Creat_powercurver(nx,ny,dx,dy,series.U[st.session_state["ind"]],series.DIR[st.session_state["ind"]],turbine_sites,D,power_curver,wakemodel,option_superpos)
			st.pyplot(fig,use_container_width=False)	
			st.session_state["ind"]= st.select_slider(
				'Select time',
				options=list(series.index))
			u_storge_all,p_storge_all,ct_storge_all=Timeseries_windfarm(turbine_sites,D,power_curver,list(series.U),list(series.DIR),wakemodel,option_superpos)
